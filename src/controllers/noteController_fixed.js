...
async function getNotes(req, res) {
  try {
    ...
    let whereConditions = ['n.user_id = ?', 'n.is_deleted = false'];
    let params = [userId];
    ...
    const [notes] = await pool.query(
      `SELECT n.id, n.title, n.content, n.category, n.is_favorite, n.word_count, 
              n.source, n.url, n.category_tag, n.created_at, n.updated_at,
...
async function createNote(req, res) {
  try {
    ...
    // 计算字数
    const calculatedWordCount = wordCount || content.replace(/\s/g, '').length;
    ...
    const [result] = await pool.query(
      `INSERT INTO notes (user_id, title, content, category, word_count, source, url, category_tag) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [userId, sanitizedTitle, sanitizedContent, category, calculatedWordCount, source, url, categoryTag]
    );
...
async function updateNote(req, res) {
  try {
    const userId = req.user.id;
    const noteId = req.params.id;
    const { title, content, category, tags, source, url, categoryTag, wordCount } = req.body;
    ...
    if (notes.length === 0) {
      return error(res, '笔记不存在或无权限访问', 404);
    }
    ...
    if (content) {
      ...
      values.push(calculatedWordCount);
    }
...
async function searchNotes(req, res) {
  try {
    ...
    const [notes] = await pool.query(
      `SELECT n.id, n.title, n.content, n.category, n.is_favorite, n.word_count, n.source, n.url, n.created_at, n.updated_at,
              GROUP_CONCAT(t.name) as tags
       FROM notes n
       LEFT JOIN note_tags nt ON n.id = nt.note_id
       LEFT JOIN tags t ON nt.tag_id = t.id
  ...
  try {
    ...
    const [notes] = await pool.query(
      `SELECT n.id, n.title, n.content, n.category, n.is_favorite, n.word_count, n.source, n.url, n.created_at, n.updated_at,
       ...
       LEFT JOIN note_tags nt ON n.id = nt.note_id
       LEFT JOIN tags t ON nt.tag_id = t.id
       WHERE n.user_id = ? AND n.category = ? AND n.is_deleted = false
       GROUP BY n.id
       ORDER BY n.updated_at DESC
       LIMIT ? OFFSET ?`,
...
async function getNotesByTag(req, res) {
  try {
    ...
    const [notes] = await pool.query(
      `SELECT DISTINCT n.id, n.title, n.content, n.category, n.is_favorite, n.word_count, n.source, n.url, n.created_at, n.updated_at,
...
async function getFavorites(req, res) {
  try {
    ...
    const [notes] = await pool.query(
      `SELECT n.id, n.title, n.content, n.category, n.word_count, n.source, n.url, n.created_at, n.updated_at,
              GROUP_CONCAT(t.name) as tags
       FROM notes n
       LEFT JOIN note_tags nt ON n.id = nt.note_id
       LEFT JOIN tags t ON nt.tag_id = t.id
       WHERE n.user_id = ? AND n.is_favorite = true AND n.is_deleted = false
       GROUP BY n.id
       ORDER BY n.updated_at DESC
       LIMIT ? OFFSET ?`,
...
async function getTrash(req, res) {
  try {
    ...
    const [notes] = await pool.query(
      `SELECT n.id, n.title, n.content, n.category, n.deleted_at, n.word_count, n.source, n.url,
              GROUP_CONCAT(t.name) as tags
       FROM notes n
       LEFT JOIN note_tags nt ON n.id = nt.note_id
       LEFT JOIN tags t ON nt.tag_id = t.id
       WHERE n.user_id = ? AND n.is_deleted = true
       GROUP BY n.id
